        ...
        if '公司代號' in table:
            ...
            for tr in tr_array:
                ...
                if len(td_array) > 15:
                    ...
                    # 所屬年度
                    iBelongY, vaild = COMMON.TryParse(
                        'int', COMMON.col_clear(td_array[3]).split('年')[0])
                    belongY = COMMON.year_RC2CE(iBelongY)
                    # 現金股利
                    earnM, vaild = COMMON.TryParse(
                        'float', COMMON.col_clear(td_array[12]))
                    # 股票股利
                    earnS, vaild = COMMON.TryParse(
        ...
        if '公司代號' in table:
            ...
            for tr in tr_array:
                ...
                if len(td_array) > 15:
                    ...
                    # 現金股利
                    earnM, vaild = COMMON.TryParse(
                        'float', COMMON.col_clear(td_array[11]))
                    # 股票股利
                    earnS, vaild = COMMON.TryParse(
                        'float', COMMON.col_clear(td_array[13]))
                    # 判斷是否有該公司當年度資料，更新/新增
                    index = (code, belongY)
...
def get_Dividend_crawl(StocksData, fromN2Now):
    ...
    for yyyy in range(syyyy, eyyyy):
        if StocksData.empty or yyyy not in StocksData.index.get_level_values(1):
            try:
                ...
                StocksData = StocksData.append(
                    crawl_dividend(yyyy, COMMON.__OTCCODE__))
                StocksData = StocksData.append(
                ...
                StocksData = StocksData.append(
                    crawl_dividendTDR(yyyy, COMMON.__OTCCODE__))
                StocksData = StocksData.append(
    ...
    if StocksData.empty!=True and len(StocksData) > oCnt:
        ...
        StocksData.sort_index().to_csv(f'{path}/dividend.csv', index_label=['公司代號', '所屬年度'])
