...
def crawl_comInfo(stocktype):
    ...
    for i in range(len(tr_array)):
        ...
        if (len(td_array) > 17):
            ...
            comCreateDate = datetime.strptime(COMMON.date_RC2CE(COMMON.col_clear(td_array[14])), '%Y/%m/%d')
            comOnDate = datetime.strptime(COMMON.date_RC2CE(COMMON.col_clear(td_array[15])), '%Y/%m/%d')
            comCapital = COMMON.col_clear(td_array[17])
            index.append(code)
            data.append([comName, comSName, comType, comCreateDate, comOnDate, comCapital])

    dfComInfo = pd.DataFrame(data=data, index=index, columns=['公司名稱', '公司簡稱', '產業類別', '成立日期', '上市日期', '資本額'])
...
def get_ComInfo_data(reload=False):
    ...
    if reload != True and os.path.exists(file):
        dfComInfo = pd.read_csv(file, index_col=0)
        return dfComInfo
    else:
